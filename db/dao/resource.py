...
class ResourceDAO(BaseDAO):
    ...
    def read_all_owned_by_type(self, type):
        objs = self.db.session.query(self.class_entity).filter_by(owned=True, type=type).all()
        return objs
